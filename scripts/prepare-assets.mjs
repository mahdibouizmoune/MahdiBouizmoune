...
for(const locale of ['en','fr','ar']){
 ...
 for(const[slug,title]of [['home',d.hero.eyebrow],...Object.entries(d.work).map(([slug,p])=>[slug,p.name+' | '+p.role])]){
  ...
  const markup='<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630"><rect width="1200" height="630" fill="#f5f3ec"/><rect x="0" y="0" width="22" height="630" fill="#b93616"/><text x="80" y="130" font-family="Arial" font-size="54" font-weight="bold" fill="#20211f">El Mahdi Bouizmoune</text>'+lines.map((s,i)=>'<text x="'+x+'" y="'+(265+i*66)+'" direction="'+(rtl?'rtl':'ltr')+'" text-anchor="'+'middle'+'" font-family="Segoe UI,Arial,sans-serif" font-size="43" fill="#20211f">'+escape(s)+'</text>').join('')+'<text x="80" y="560" font-family="Arial" font-size="25" fill="#62645b">Safi, Morocco · '+locale.toUpperCase()+'</text></svg>';
