...
for(const locale of ['en','fr','ar']){
 ...
 for(const[slug,title]of [['home',d.hero.eyebrow],...Object.entries(d.work).map(([slug,p])=>[slug,p.name+' | '+p.role])]){
  ...
  const markup='<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630"><defs><pattern id="grid" width="48" height="48" patternUnits="userSpaceOnUse"><path d="M 48 0 L 0 0 0 48" fill="none" stroke="#8894C2" stroke-width="1"/></pattern></defs><rect width="1200" height="630" fill="#4E598C"/><rect width="1200" height="630" fill="url(#grid)"/><rect x="0" y="0" width="16" height="630" fill="#FFD166"/><text x="80" y="120" font-family="Arial" font-size="36" font-weight="bold" fill="#FFD166">El Mahdi Bouizmoune</text>'+lines.slice(0,4).map((s,i)=>'<text x="'+x+'" y="'+(245+i*66)+'" direction="'+(rtl?'rtl':'ltr')+'" text-anchor="middle" font-family="Segoe UI,Arial,sans-serif" font-size="43" fill="#FCEFC7">'+escape(s)+'</text>').join('')+'<text x="80" y="570" font-family="Arial" font-size="22" fill="#E3E7F5">Digital Marketing Manager · '+locale.toUpperCase()+'</text></svg>';
