...
await fs.writeFile('public/site.webmanifest',JSON.stringify({name:SITE.name,short_name:'Mahdi',start_url:'/',display:'standalone',background_color:'#EDE8D6',theme_color:'#242424',icons:[192,512].map(size=>({src:'/icon-'+size+'.png',sizes:size+'x'+size,type:'image/png'}))},null,2));
...
for(const locale of ['en','fr','ar']){
 ...
 for(const[slug,title]of [['home',d.hero.eyebrow],...Object.entries(d.work).map(([slug,p])=>[slug,p.name+' | '+p.role])]){
  ...
  const markup='<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630"><defs><pattern id="grid" width="48" height="48" patternUnits="userSpaceOnUse"><path d="M 48 0 L 0 0 0 48" fill="none" stroke="#3D2740" stroke-width="1"/></pattern></defs><rect width="1200" height="630" fill="#241528"/><rect width="1200" height="630" fill="url(#grid)"/><rect x="0" y="0" width="16" height="630" fill="#D9A441"/><text x="80" y="120" font-family="Arial" font-size="36" font-weight="bold" fill="#D9A441">El Mahdi Bouizmoune</text>'+lines.slice(0,4).map((s,i)=>'<text x="'+x+'" y="'+(245+i*66)+'" direction="'+(rtl?'rtl':'ltr')+'" text-anchor="middle" font-family="Segoe UI,Arial,sans-serif" font-size="43" fill="#F0E6E8">'+escape(s)+'</text>').join('')+'<text x="80" y="570" font-family="Arial" font-size="22" fill="#B7A0BC">Digital Marketing Manager · '+locale.toUpperCase()+'</text></svg>';
