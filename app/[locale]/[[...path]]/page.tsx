...
export async function generateMetadata({params}:Props){const{locale,path=[]}=await params;if(!isLocale(locale)||locale==='en')return {robots:{index:false}};const d=dictionary(locale),g=growth(locale),p=d.work[path[1] as keyof typeof d.work];return path.length===0?pageMetadata('El Mahdi Bouizmoune | Digital Marketing Manager',g.subline,'/',locale):path[0]==='clients'&&path.length===1?pageMetadata(d.nav.clients+' | El Mahdi Bouizmoune',g.clientsDescription,'/clients',locale):path[0]==='work'&&path.length===1?pageMetadata(g.allWork+' | El Mahdi Bouizmoune',g.workDescription,'/work',locale):p&&path[0]==='work'&&path.length===2?pageMetadata(getCase(path[1] as CaseSlug,locale).client+' | El Mahdi Bouizmoune',p.intro,'/work/'+path[1],locale):{title:d.meta.notFound+' | El Mahdi Bouizmoune',robots:{index:false}};}
