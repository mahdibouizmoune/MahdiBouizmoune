...
export async function POST(request:Request){
 ...
 if(!signature||!/^[a-f0-9]{64}$/.test(signature)||!timingSafeEqual(Buffer.from(signature),Buffer.from(signed)))return json({error:'invalidSubmission'},400);
 ...
 try{
  const result=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:'Bearer '+process.env.RESEND_API_KEY,'Content-Type':'application/json','Idempotency-Key':'portfolio-'+nonce},body:JSON.stringify({from:process.env.CONTACT_FROM_EMAIL,to:[process.env.CONTACT_TO_EMAIL||SITE.email],reply_to:String(data.email).trim(),subject:'Portfolio enquiry',text:'Name: '+String(data.name).trim()+'\nEmail: '+String(data.email).trim()+'\nCompany: '+(typeof data.company==='string'?data.company:'').trim()+'\n\n'+String(data.message).trim()}),signal:AbortSignal.timeout(12000)});
