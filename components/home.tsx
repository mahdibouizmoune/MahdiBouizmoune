...
import {Hero,LogoStrip,SelectedWork,BeforeAfter,Services,Process,Reviews,About,WaysToWork,FAQ} from './home-sections';
...
export function Home({locale='en'}:{locale?:Locale}){
 ...
 return <><Header locale={locale}/><main id="main" tabIndex={-1}><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(person).replace(/</g,'\\u003c')}}/><Hero locale={locale}/><LogoStrip locale={locale}/><SelectedWork locale={locale}/><BeforeAfter locale={locale}/><Services locale={locale}/><Process locale={locale}/><Reviews locale={locale}/><About locale={locale}/><WaysToWork locale={locale}/><FAQ locale={locale}/><Contact locale={locale}/></main><Footer locale={locale}/></>;
}
