...
export function ContactForm({copy,locale}:{copy:Dictionary['form'];locale:Locale}){
 ...
 const token=useRef('');
 useEffect(()=>{fetch('/api/contact',{cache:'no-store'}).then(r=>r.ok?r.json():null).then(data=>{token.current=(data as {token?:string}|null)?.token||'';}).catch(()=>{});},[]);
 async function submit(e:React.FormEvent<HTMLFormElement>){
  ...
  try{const response=await fetch('/api/contact',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...data,locale,token:token.current})});const result=await response.json() as {fields?:Record<string,string>;error?:string};
   ...
   else{setStatus(copy.success);form.reset();track('contact_form_submit',{locale});const fresh=await fetch('/api/contact',{cache:'no-store'});token.current=fresh.ok?(await fresh.json() as {token:string}).token:'';}
