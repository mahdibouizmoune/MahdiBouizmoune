...
export function ContactForm({copy,locale}:{copy:Dictionary['form'];locale:Locale}){
 ...
 async function submit(e:SubmitEvent<HTMLFormElement>){
  ...
  for(const name of ['name','email','message'])if(!str(data[name]).trim())next[name]=copy.required;
  if(data.email&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(str(data.email)))next.email=copy.invalidEmail;
  if(data.message&&str(data.message).trim().length<10)next.message=copy.tooShort;
 ...
 <p className="form-privacy">{copy.privacy}</p><button type="submit" className="button contact-button" disabled={pending}>{pending?copy.pending:copy.submit}</button><output className="form-status" aria-live="polite" aria-atomic="true">{status}</output><noscript><p>{copy.noJs}</p></noscript>
 </form>
 );
}
export function EmailLink({parts,label}:{parts:string[];label:string}){
 const[email,setEmail]=useState('');
 // oxlint-disable-next-line react/react-compiler -- The address is only assembled client-side, after mount, so scrapers reading the static/SSR HTML never see a joined mailto address; parts stays split until then.
 useEffect(()=>setEmail(parts.join('@')),[parts]);
