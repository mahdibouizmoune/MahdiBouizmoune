...
export function ExpandableList({initial,more,seeMore,showLess}:{initial:string[];more:string[];seeMore:string;showLess:string}){
 const [expanded,setExpanded]=useState<boolean|null>(null);
 // oxlint-disable-next-line react/react-compiler -- Progressive enhancement: without JS the full list must render (expanded===null), so the default state can't already be false on first paint. This effect collapses it to just the initial items once hydration confirms JS is running.
 useEffect(()=>setExpanded(false),[]);
 const showAll=expanded===null||expanded;
 return <div className="brand-list">{initial.map(n=><bdi key={n}>{n}</bdi>)}{showAll&&more.map(n=><bdi key={n}>{n}</bdi>)}{expanded!==null&&<button type="button" className="text-link brand-toggle" aria-expanded={expanded} onClick={()=>setExpanded(!expanded)}>{expanded?showLess:seeMore}</button>}</div>;
}
